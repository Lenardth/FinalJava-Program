...
public class MainMenu extends JFrame {
    ...
    public MainMenu(double initialBalance) {
        ...
        JMenuItem altThemeMenuItem = new JMenuItem("Switch to Alt Theme");
        editMenu.add(themeMenuItem);
        editMenu.add(altThemeMenuItem);
        menuBar.add(editMenu);

        JMenu helpMenu = new JMenu("Help");
        JMenuItem aboutMenuItem = new JMenuItem("About");
        JMenuItem adminAnalyticsMenuItem = new JMenuItem("Admin Analytics");
        helpMenu.add(aboutMenuItem);
        helpMenu.add(adminAnalyticsMenuItem);
        ...
        JButton emergencyContactButton = createModernSidebarButton("Emergency Contact", BUTTON_COLOR);
        ...
        sidebarPanel.add(Box.createRigidArea(new Dimension(0, 10)));
        sidebarPanel.add(emergencyContactButton);
        ...
        bookTaxiButton.addActionListener(e -> {
            ...
            if (initialPoint != null && finalDestination != null && !initialPoint.isEmpty() && !finalDestination.isEmpty()) {
                ...
                updateBookingTrends(initialPoint, finalDestination);
                isFarePaid = false;
            } else {
        ...
        nearbyTaxiButton.addActionListener(e -> {
            StringBuilder result = new StringBuilder("Nearby Taxis:\n");
            for (Taxi taxi : taxis) {
                result.append("Taxi Number: ").append(taxi.getTaxiNumber())
                        .append(", Start Time: ").append(taxi.getStartTime())
                        .append(", End Time: ").append(taxi.getEndTime()).append("\n");
            }
            if (result.length() == 0) {
                JOptionPane.showMessageDialog(this, "No nearby taxis available.", "Nearby Taxis", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(this, result.toString(), "Nearby Taxis", JOptionPane.INFORMATION_MESSAGE);
            }
        });
        ...
        emergencyContactButton.addActionListener(e -> {
            JOptionPane.showMessageDialog(this, "Emergency Contact: 0800-123-456\nAvailable 24/7 for assistance.", "Emergency Contact", JOptionPane.INFORMATION_MESSAGE);
        });
        ...
        withdrawMoneyButton.addActionListener(e -> {
            if (isFarePaid) {
                JOptionPane.showMessageDialog(this, "Fare has already been paid.", "Payment Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
    ...
    private void switchAltTheme() {
        isAltTheme = !isAltTheme;
        isLightTheme = false;
        Color panelBackground = isAltTheme ? BG_COLOR_ALT : BG_COLOR_DARK;
        Color labelForeground = TEXT_COLOR;
        Color buttonBackground = BUTTON_COLOR_ALT;
        Color buttonForeground = BUTTON_TEXT_COLOR;
    ...
    private void updateBookingTrends(String initialPoint, String finalDestination) {
        String route = initialPoint + " - " + finalDestination;
        bookingTrends.put(route, bookingTrends.getOrDefault(route, 0) + 1);
    }

    private void showAdminAnalytics() {
        if (bookingTrends.isEmpty()) {
            JOptionPane.showMessageDialog(this, "No booking data available.", "Admin Analytics", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        StringBuilder analytics = new StringBuilder("Booking Trends: ");
        for (Map.Entry<String, Integer> entry : bookingTrends.entrySet()) {
            analytics.append(entry.getKey()).append(": ").append(entry.getValue()).append(" bookings ");
        }
        JOptionPane.showMessageDialog(this, analytics.toString(), "Admin Analytics", JOptionPane.INFORMATION_MESSAGE);
    }
